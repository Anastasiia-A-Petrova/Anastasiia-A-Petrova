# Hi, I'm Anastasiia 👋

I am a Junior Data Analyst building end-to-end analytics projects using Python, SQL, Excel and BI tools.

My projects cover the full analytics workflow: data collection, data cleaning, transformation, exploratory analysis, KPI calculation, business analysis and dashboard development.

## 🛠 Tech Stack

![Python](https://img.shields.io/badge/Python-3.14-blue?logo=python)
![SQL](https://img.shields.io/badge/SQL-Analytics-orange)
![Excel](https://img.shields.io/badge/Excel-Analytics-green?logo=microsoftexcel)
![Power BI](https://img.shields.io/badge/Power%20BI-Dashboard-yellow?logo=powerbi)
![BigQuery](https://img.shields.io/badge/BigQuery-Google%20Cloud-blue?logo=googlecloud)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Database-blue?logo=postgresql)
![Pandas](https://img.shields.io/badge/Pandas-Data%20Analysis-black?logo=pandas)
![n8n](https://img.shields.io/badge/n8n-Workflow%20Automation-orange?logo=n8n)

---

My main interests:

* product analytics
* user behavior analysis
* data automation and analytics pipelines
* market analysis
* content analytics
* data visualization
* business-oriented reporting

---

## 🛠 Skills

**Analytics & SQL**

* SQL
* PostgreSQL
* BigQuery
* CTEs
* Window Functions
* Views
* Exploratory Data Analysis
* KPI analysis
* Business analysis

**Python**

* Pandas
* NumPy
* Data cleaning and preprocessing
* Feature Engineering
* REST APIs
* Automated data collection

**Excel**

* Excel 2019
* Power Query
* Data transformation
* Pivot Tables
* KPI calculations
* Data validation
* Dashboard development
* Business-oriented reporting

**Visualization**

* Power BI
* DAX
* Excel Dashboards
* Matplotlib
* Seaborn

**Data Collection**

* REST APIs
* Steam APIs
* YouTube Data API
* Telegram API
* Web data collection

**Automation**

* n8n
* Workflow automation
* Scheduled data collection
* API integration
* Pipeline orchestration
* Automated database loading

---

# 📊 Featured Projects

## 🛒 GA4 Ecommerce Analytics

BigQuery SQL | GA4 | Power BI

End-to-end ecommerce analytics project using Google Analytics 4 event data, BigQuery SQL and Power BI.

<img src="https://raw.githubusercontent.com/Anastasiia-A-Petrova/ga4-ecommerce-analytics/main/images/customer_and_funnel_analysis.png" width="700" alt="GA4 Ecommerce Analytics Dashboard">

**Highlights:**

* 4.3M+ GA4 events analyzed
* Data exploration and quality validation
* Ecommerce KPI calculation (revenue, transactions, AOV)
* Customer segmentation and retention analysis
* Product performance analysis
* Marketing channel and audience analysis
* Interactive Power BI dashboard with 4 analytical pages

🔗 Repository: [ga4-ecommerce-analytics](https://github.com/Anastasiia-A-Petrova/ga4-ecommerce-analytics)

---

## 📱 Telegram Channel Analytics

Python | Telethon | PostgreSQL | Power BI

End-to-end content analytics project focused on Telegram channel performance and audience engagement.

<img src="https://raw.githubusercontent.com/Anastasiia-A-Petrova/telegram-channel-analysis/main/images/overview.png" width="700" alt="Telegram Channel Analytics Dashboard">

**Highlights:**

* Collected and processed 1,000+ Telegram posts
* Built a data collection pipeline using Telethon API
* Analyzed content formats, engagement metrics and publishing patterns
* Performed channel comparison analysis
* Built an interactive Power BI dashboard

🔗 Repository: [telegram-channel-analysis](https://github.com/Anastasiia-A-Petrova/telegram-channel-analysis)

---

## 🤖 Automated Gacha Game Data Pipeline

Python | REST APIs | PostgreSQL | n8n

Automated data collection pipeline for gacha game analytics, combining API-based data collection, Python processing, PostgreSQL storage and workflow automation.

<img src="https://raw.githubusercontent.com/Anastasiia-A-Petrova/steam-gacha-activity-monitor/main/images/pipeline.png" width="700" alt="Steam Gacha Activity Pipeline">

<img src="https://raw.githubusercontent.com/Anastasiia-A-Petrova/steam-gacha-activity-monitor/main/images/dashboard.png" width="700" alt="Steam Gacha Activity Dashboard">

**Highlights:**

* automated data collection from external APIs;
* built a Python data collection service;
* processed and transformed raw API data;
* loaded structured data into PostgreSQL;
* orchestrated the workflow using n8n;
* implemented pipeline status monitoring;
* designed the project as a repeatable data collection workflow.

🔗 Repository: [steam-gacha-activity-monitor](https://github.com/Anastasiia-A-Petrova/steam-gacha-activity-monitor)

---

## 🎸 YouTube Channel Analytics

Python | PostgreSQL | YouTube API | Power BI

End-to-end analysis of a YouTube channel (@Gleb_Oleinik) using YouTube Data API, Python, SQL and Power BI.

<img src="https://raw.githubusercontent.com/Anastasiia-A-Petrova/youtube-analytics-gleb-oleynik/main/images/overview.png" width="700" alt="YouTube Channel Analytics Dashboard">

**Highlights:**

* 754 videos analyzed
* API data collection pipeline
* Engagement metrics calculation
* Content segmentation
* Interactive Power BI dashboard

🔗 Repository: [youtube-analytics-gleb-oleynik](https://github.com/Anastasiia-A-Petrova/youtube-analytics-gleb-oleynik)

---

## 🎮 Steam Market Opportunity Analysis

Python | Steam APIs | Excel | Power Query

End-to-end market analysis project evaluating Steam game segments to identify opportunities for game developers with limited resources.

<img src="https://raw.githubusercontent.com/Anastasiia-A-Petrova/steam-market-opportunity-analysis/main/images/dashboard.png" width="700" alt="Steam Market Opportunity Analysis Dashboard">

**Highlights:**

* analyzed ~1,000 Steam games released between 2024 and 2026;
* collected data using Steam Store API, Steam Web API and SteamSpy;
* built a Python data collection and preprocessing pipeline;
* analyzed market demand, competition and price barriers;
* created a composite Opportunity Score;
* evaluated market size using estimated owners;
* performed sensitivity analysis of model weights;
* built an interactive Excel dashboard using Power Query and Excel formulas;
* identified Massively Multiplayer as the strongest overall market opportunity.

🔗 Repository: [steam-market-opportunity-analysis](https://github.com/Anastasiia-A-Petrova/steam-market-opportunity-analysis)

---

## 👮‍♂️ Toronto Hate Crime Analysis

Python | PostgreSQL | GeoPandas | Power BI

End-to-end analysis of Toronto Police hate crime open data.

<img src="https://raw.githubusercontent.com/Anastasiia-A-Petrova/Toronto-Hate-Crime-Analysis/main/images/bias.png" width="700" alt="Toronto Hate Crime Analysis Dashboard">

**Highlights:**

* cleaned and transformed 2,000+ records;
* normalized bias category data;
* created analytical SQL views;
* performed neighbourhood-level geospatial analysis;
* built an interactive Power BI dashboard.

🔗 Repository: [Toronto-Hate-Crime-Analysis](https://github.com/Anastasiia-A-Petrova/Toronto-Hate-Crime-Analysis)

---

## 🤖 HH.ru Job Search Telegram Bot

Python | Telegram Bot API | hh.ru API | Render

Telegram bot for searching vacancies through hh.ru API.

<img src="https://raw.githubusercontent.com/Anastasiia-A-Petrova/hh-job-bot/main/images/start_menu.png" width="300" alt="HH Job Search Telegram Bot">

**Highlights:**

* Telegram bot interface with inline keyboards
* API-based vacancy search
* Profession, city and work format filters
* Pagination of search results
* Custom profession queries
* Cloud deployment with Render Webhook

🔗 Repository: [hh-job-bot](https://github.com/Anastasiia-A-Petrova/hh-job-bot)

---

## 🌱 Currently Exploring

* Advanced SQL techniques
* Product analytics frameworks
* Unit economics
* Statistics for business analysis
* Advanced data visualization

---

## 📫 Contacts

Email: [anastasiiapetrovam1@gmail.com](mailto:anastasiiapetrovam1@gmail.com)

LinkedIn: [linkedin.com/in/anastasiia-a-petrova](https://www.linkedin.com/in/anastasiia-a-petrova)

